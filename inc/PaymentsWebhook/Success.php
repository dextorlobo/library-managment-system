...
class Success {
	...
	public function init() {
		...
		add_action( 'thl_payment_success_callback', array( $this, 'thl_payment_success_method' ), 10, 2 );
	}

	public function thl_payment_success_method( $palyload, $headers ) {
		...
		$order_details['expiry_date']    = get_post_meta( $merchantTransactionId[0], 'expiry_date', true );
		$order_details['mail_status']    = get_post_meta( $merchantTransactionId[0], 'mail_status', true );
		update_post_meta( $order_details['seat_no'], 'thl_available', 'occupied' );
		update_post_meta( $order_details['seat_no'], 'thl_expiration_date', $order_details['expiry_date'] );

		do_action( 'thl_payment_success_after_update_status', $order_details );
	}
}
